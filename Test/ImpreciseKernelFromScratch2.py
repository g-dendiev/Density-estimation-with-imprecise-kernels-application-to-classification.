...
# ICI ON FAIT DE L'INTERVALLE DOMINANCE MODIFIEE ET FONCTIONNELLE ET C'EST PAS OUF D'UN POINT DE VUE THEORIQUE !
...
def predictImpreciseKernel(dataset, columnWithClassResponse, inputVector, margeEpsilon):
				...
				if classValueHight not in dominatedClass:
					if classValueHight != classValueLow:
						...
						if lowProba > hightProba and tjrsSup == 1:
							#Si on entre ici c'est que la classe avec la proba haute est dominée
							# par la classe avec la proba basse donc on enlève la possibilité d'avoir cette classe (ProbaHaute)
	...
	if (bestLabel == []):
		...
		for classValueHight2, hightProba2 in hightProbabilities.items():
			if (classValueHight2 not in dominatedClass):
				...
				for classValueLow2, lowProba2 in lowProbabilities.items():
					if classValueLow2 not in dominatedClass:
